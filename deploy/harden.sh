...
echo "    2. OCI Security List: add ingress rules for TCP 80,443 from 0.0.0.0/0"
echo "       (OCI Console → Networking → VCN → Security Lists)"
echo "    3. Disable ubuntu user: sudo deluser ubuntu sudo && sudo rm /home/ubuntu/.ssh/authorized_keys"
echo "    4. Install Docker:  curl -fsSL https://get.docker.com | sh"
echo "    5. Add user to docker group:  usermod -aG docker $USERNAME"
echo "    6. Install nginx:  sudo apt install -y nginx"
echo ""
