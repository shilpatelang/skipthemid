...
- **2026-05-08 — Phase 1.6: Map dish thumbnails + hero polish + canonicals.** Map markers now show circular dish thumbnails (canvas-cropped at load time, amber ring) via Mapbox `addImage` + symbol layer; dishes without an image fall back to the original pulsing dot. Landing hero redesigned: map enlarged to 75vh at zoom 1.5 so the full globe is visible; overlay reduced to just the Explore Full Map CTA; visible H1 replaced with `sr-only` H1 to preserve SEO. Featured Dishes count bumped 6 → 20. Removed dull text-only hover popup (click still routes to dish page). Added canonical metadata on `/` and `/dish/[slug]` to fix Search Console "Duplicate without user-selected canonical" (4 URLs). `AbortController` on /api/dishes fetch + `addImage` map-validity guard fix a StrictMode-induced terrain crash. Branch: `phase-1.6/map-thumbnails`.

**Manual follow-ups after deploy:** submit sitemap to Google Search Console + Bing Webmaster Tools. Validate Recipe rich results at search.google.com/test/rich-results. Configure Cloudflare redirects (www→non-www, force HTTPS) to fully resolve the duplicate-URL Search Console finding.
